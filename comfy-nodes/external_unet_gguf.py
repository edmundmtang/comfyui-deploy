...
class ComfyUIDeployExternalUnetGGUF:
    @classmethod
    def INPUT_TYPES(s):
        return {
            "required": {
                "input_id": (
                    "STRING",
                    {"multiline": False, "default": "input_unet"},
                ),
            },
            "optional": {
                "default_value": (folder_paths.get_filename_list("unet_gguf"), ),
                "display_name": (
                    "STRING",
    ...
    def run(self, input_id, default_value=None, display_name=None, description=None):
        ...
        if default_value.startswith('http'):
            unique_filename = str(uuid.uuid4()) + ".gguf"
...
NODE_CLASS_MAPPINGS = {
    "ComfyUIDeployExternalUnetGGUF": ComfyUIDeployExternalUnetGGUF}
NODE_DISPLAY_NAME_MAPPINGS = {
    "ComfyUIDeployExternalUnetGGUF": "External Unet (GGUF) (ComfyUI Deploy)"}
